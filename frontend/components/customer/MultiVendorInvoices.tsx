...
export default function MultiVendorInvoices({ order, onClose }: MultiVendorInvoicesProps) {
  ...
  order.items.forEach(item => {
    ...
    group.tax += parseFloat(item.tax_amount);
    group.total += parseFloat(item.total_amount);
  });

  vendorMap.forEach(group => vendorGroups.push(group));

  // Calculate shipping per vendor (split equally)
  const shippingPerVendor = parseFloat(order.shipping_charge) / vendorGroups.length;
      ...
      <body>
        <div class="invoice-container">
          ...
          <div class="payment-info">
            ...
            ${vendorGroups.length > 1 ? `<p><strong>Note:</strong> Payment for entire order (₹${parseFloat(order.total_amount).toLocaleString('en-IN')}) was made together. This invoice shows only items from this vendor.</p>` : ''}
          </div>
                ...
                <div className="space-y-2 mb-4">
                  {vendorGroup.items.map((item, itemIndex) => (
                    <div key={itemIndex} className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-900">₹{parseFloat(item.total_amount).toLocaleString('en-IN')}</span>
                    </div>
                  ))}
                </div>
