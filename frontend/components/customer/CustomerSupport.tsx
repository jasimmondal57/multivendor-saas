...
export default function CustomerSupport() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await api.get('/v1/customer/support/categories');
  ...
  const fetchTickets = async () => {
    setLoading(true);
    try {
      const response = await api.get('/v1/customer/support/tickets');
      if (response.data.success) {
  ...
  const fetchTicketDetails = async (ticketId: number) => {
    try {
      const response = await api.get(`/v1/customer/support/tickets/${ticketId}`);
      if (response.data.success) {
  ...
  const createTicket = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/v1/customer/support/tickets', formData);
      if (response.data.success) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await api.post(`/v1/customer/support/tickets/${selectedTicket.id}/messages`, {
        message: newMessage,
      });
